...
function IssueCard({
  ...
  return (
    ...
    >
      ...
      {/* Sync status badge */}
      {issue.internal_only ? (
        <span className="inline-block text-[10px] px-1.5 py-0.5 rounded bg-zinc-500/10 text-zinc-500 mb-2">
          Internal
        </span>
      ) : issue.taskhive_sync_status === 'synced' ? (
        <span className="inline-block text-[10px] px-1.5 py-0.5 rounded bg-green-500/10 text-green-600 mb-2">
          Synced
        </span>
      ) : issue.taskhive_sync_status === 'pending' ? (
        <span className="inline-block text-[10px] px-1.5 py-0.5 rounded bg-yellow-500/10 text-yellow-600 mb-2">
          Pending
        </span>
      ) : issue.taskhive_sync_status === 'failed' ? (
        <span className="inline-block text-[10px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-600 mb-2">
          Failed
        </span>
      ) : null}
